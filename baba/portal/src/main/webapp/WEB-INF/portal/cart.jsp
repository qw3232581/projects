...
<script type="text/javascript">
	...
	//结算
	function trueBuy(){
        var size = $("input[name='skuIds']:checked").size();
        if (size == 0) {
            alert('请至少选择一个商品进行结算');
            return;
        }
...
<div class="w w1 header clearfix">
    <div id="logo">
        <a href="/"><img src="/images/XBB2.png" title="返回新巴巴商城首页" alt="返回新巴巴商城首页"></a>
    </div>
    <div class="language">
...
<c:if test="${fn:length(finalCart.items) != 0}">
<div class="w cart">
    <div class="cart-hd group">
        <h2>我的购物车</h2>
    </div>

<form id="jvForm" action="/buyer/checkOut" method="post">
    <div id="show">
        ...
        <div class="cart-inner">
            ...
            <div id="product-list" class="cart-tbody">
                ...
                <!-- ************************商品开始********************* -->
                <c:forEach items="${finalCart.items}" var="item">
                    <div id="product_${item.skuId}" class="item item_selected ">
                        <div class="item_form clearfix">

                            <div class="cell p-checkbox">
                                <input class="checkbox" type="checkbox" name="skuIds" value="${item.skuId}">
                            </div>

                            <div class="cell p-goods">
                                <div class="p-img">
                                    <a href="javascript:;" target="_blank">
                                        <img src="${item.sku.imgUrl}" alt="${item.sku.productName}" width="52" height="52">
                                    </a>
                                </div>
                                <div class="p-name">
                                    <a href="javascrip:;"> ${item.sku.productName}--${item.sku.colorName}-- ${item.sku.size }</a>
                                </div>
                            </div>
                            <div class="cell p-price">
                                <span class="price"> ¥${item.sku.skuPrice} </span>
                            </div>
            ...
            <div class="cart-toolbar clearfix">
                <div class="total fr">
                    <p><span class="totalSkuPrice">¥${finalCart.productPrice }</span>商品金额：</p>
                    <p><span id="totalRePrice">¥${finalCart.fee }</span>运费：</p>
                </div>

                <div class="amout fr">
                    <span id="selectedCount">${finalCart.productAmount }</span>件商品
                </div>
            </div>
            <div class="ui-ceilinglamp-1" style="width: 988px; height: 49px;">
                <div class="cart-dibu ui-ceilinglamp-current" style="width: 988px; height: 49px;">
                    ...
                    <div class="cart-total-2014">
                        ...
                        <div class="total fr">
                            总计:<span class="totalSkuPrice">¥${finalCart.totalPrice }</span>
                        </div>
                    </div>
                </div>
            </div>
